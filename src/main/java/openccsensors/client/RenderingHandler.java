...
public class RenderingHandler implements ISimpleBlockRenderingHandler {
	...
	public void renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer) {
		...
		if (block == OpenCCSensors.Blocks.sensorBlock) {
			TileEntityRendererDispatcher.instance.renderTileEntityAt(new TileEntitySensor(), 0.0D, 0.0D, 0.0D, 0.0F);
			GL11.glEnable(GL12.GL_RESCALE_NORMAL);
		}
		if (block == OpenCCSensors.Blocks.gaugeBlock) {
			TileEntityRendererDispatcher.instance.renderTileEntityAt(new TileEntityGauge(), 0.0D, 0.0D, 0.0D, 0.0F);
			GL11.glEnable(GL12.GL_RESCALE_NORMAL);
		}
	}
