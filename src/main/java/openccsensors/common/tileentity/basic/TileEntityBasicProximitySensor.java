...
public class TileEntityBasicProximitySensor extends TileEntity implements IBasicSensor {
	...
	public void updateEntity() {
		...
		if (!worldObj.isRemote) {
			...
			distance = OpenCCSensors.Sensors.proximitySensor.getDistanceToNearestEntity(worldObj, blockPos, entityMode, owner);
			...
			if (flag) {
				...
				worldObj.notifyBlockOfNeighborChange(xCoord, yCoord, zCoord, OpenCCSensors.Blocks.basicSensorBlock);
				worldObj.notifyBlockOfNeighborChange(xCoord, yCoord - 1, zCoord, OpenCCSensors.Blocks.basicSensorBlock);
				worldObj.notifyBlockOfNeighborChange(xCoord, yCoord + 1, zCoord, OpenCCSensors.Blocks.basicSensorBlock);
				worldObj.notifyBlockOfNeighborChange(xCoord - 1, yCoord, zCoord, OpenCCSensors.Blocks.basicSensorBlock);
				worldObj.notifyBlockOfNeighborChange(xCoord + 1, yCoord, zCoord, OpenCCSensors.Blocks.basicSensorBlock);
				worldObj.notifyBlockOfNeighborChange(xCoord, yCoord, zCoord - 1, OpenCCSensors.Blocks.basicSensorBlock);
				worldObj.notifyBlockOfNeighborChange(xCoord, yCoord, zCoord + 1, OpenCCSensors.Blocks.basicSensorBlock);
