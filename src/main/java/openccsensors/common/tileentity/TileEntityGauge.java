...
public class TileEntityGauge extends TileEntity implements IPeripheral {
	...
	private final HashMap tileProperties = new HashMap();
	private final CallbackEventManager eventManager = new CallbackEventManager();
	...
	public Object[] callMethod(IComputerAccess computer, ILuaContext context, int method, Object[] arguments) throws Exception {
		return new Object[] { eventManager.queueMethodCall(computer, method, arguments) };
	}
			...
			if (behindTile != null) {
				for (IGaugeSensor gaugeSensor : gaugeSensors) {
					if (gaugeSensor.isValidTarget(behindTile)) {
						HashMap details = gaugeSensor.getDetails(worldObj, behindTile, new ChunkCoordinates(behindTile.xCoord, behindTile.yCoord,
								behindTile.zCoord), true);
