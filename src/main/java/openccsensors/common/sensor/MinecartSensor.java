...
public class MinecartSensor implements ISensor, IRequiresIconLoading {
	...
	public HashMap getDetails(World world, Object obj, ChunkCoordinates sensorPos, boolean additional) {
		...
		/*
		 * TODO: if (Mods.RAIL) { if (minecart instanceof IEnergyTransfer) { response.putAll(RailcraftUtils.getEnergyDetails(minecart)); }
		 * 
		 * if (minecart instanceof IExplosiveCart) { response.putAll(RailcraftUtils.getExplosiveDetails(minecart)); }
		 * 
		 * if (minecart instanceof IRoutableCart) { response.put("RouteDestination", ((IRoutableCart) minecart).getDestination()); } }
		 */
		return response;
	}
