		...
		for (int x = -distance; x <= distance; x++) {
			for (int y = -distance; y <= distance; y++) {
				for (int z = -distance; z <= distance; z++) {
					...
					if (b != null
							&& (b instanceof BlockCrops || b instanceof BlockNetherWart || b instanceof BlockStem || b instanceof BlockPumpkin || b instanceof BlockMelon)) {
						CropTarget potentialTarget = new CropTarget(tileX, tileY, tileZ, b, world.getBlockMetadata(tileX, tileY, tileZ));
